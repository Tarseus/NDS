...
mkdir -p "$RUN_DIR"
{
  ...
  echo "command=run_full_lns_transfer.py --mode top8 --instances 24 --iterations 200 --rollout-size 32"
...
"$PYTHON_BIN" run_full_lns_transfer.py \
  ...
  --rollout-size 32 \
  --seed 20260809 \
  --random-code-seed 20260810 \
  --code-seed 20260803 \
  --num-processes 8 \
  2>&1 | tee "$RUN_DIR/log.txt"

"$PYTHON_BIN" analyze_portfolio_lns_transfer.py "$RUN_DIR" \
