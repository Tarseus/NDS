...
mkdir -p "$RUN_DIR"
{
  ...
  echo "command=run_crossdist_code_transfer.py --instances 64 --num-codes 32 --replicas 4 --seed 20260804"
...
"$PYTHON_BIN" run_crossdist_code_transfer.py \
  --output "$RUN_DIR" \
  --instances 64 \
  --num-codes 32 \
  --replicas 4 \
...
"$PYTHON_BIN" analyze_multisource_transfer.py "$RUN_DIR" \
  --calibration-instances 16 \
